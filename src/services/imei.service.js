...
export async function validateIMEI(imei, options = {}) {
  const clean = imei.replace(/\s|-/g, '')
  const errors = []
  const warnings = []
  ...
  if (format.ok) {
    ...
    if (imeiInfoResult.ok && imeiInfoResult.data) {
      tac = {
        ...
        manufacturer: imeiInfoResult.data.manufacturer,
        source: 'api',
      }
    } else {
...
export function quickValidateIMEI(imei) {
  const clean = imei.replace(/\s|-/g, '')
