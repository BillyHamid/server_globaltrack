...
async function computeAdminRecoveryMetrics() {
  ...
  for (const row of byStatusRaw) {
    const key = /** @type {keyof typeof byStatus} */ (row.paymentStatus)
...
router.get('/employees/:vendorId/activity', async (req, res, next) => {
  try {
...
router.get('/employees/:vendorId/sales', async (req, res, next) => {
  try {
    const { vendorId } = req.params
    const limit = Math.min(parseInt(String(req.query.limit ?? '50'), 10) || 50, 200)
    const offset = parseInt(String(req.query.offset ?? '0'), 10) || 0
...
router.get('/employees/:vendorId/metrics', async (req, res, next) => {
  try {
    const { vendorId } = req.params
...
router.get('/dashboard/recovery-metrics', async (_req, res, next) => {
  try {
