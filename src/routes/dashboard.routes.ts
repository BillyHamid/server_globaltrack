...
function parsePhotosJson(photos: string) {
  try {
...
/** GET /api/dashboard — bundle unique (remplace 6 appels parallèles côté client) */
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const raw = parseInt(String(req.query.limit ?? ''), 10)
    const limit = Number.isFinite(raw) && raw > 0
      ? Math.min(raw, MAX_BUNDLE_LIMIT)
    ...
    const movementsOut = movements.map(m => {
      const pb = m.performedBy as { id: string; name: string }
      return {
        ...m,
        performedBy: pb?.id ?? (m as { performedById: string }).performedById,
      }
    })
...
router.get('/chart-data', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const now = new Date()
