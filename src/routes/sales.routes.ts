...
const createSaleSchema = z.object({
  ...
  /** Prix catalogue connu côté client (sinon = sellingPrice du téléphone côté serveur). */
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { type, paymentStatus, clientId, search, overdue, page = '1', limit = '50' } = req.query as Record<string, string>
...
router.post('/:id/payments', validate(addPaymentSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { depositProof } = req.body as z.infer<typeof patchPaymentDepositProofSchema>
