...
exports.quickValidateIMEI = quickValidateIMEI;
const axios_1 = __importDefault(require("axios"));
const prisma_1 = require("../lib/prisma");
const logger_1 = require("../lib/logger");
// ─── Couche 1 — Format ────────────────────────────────────────────────────────
function checkFormat(imei) {
...
async function fetchTACFromIMEICheck(imei) {
    try {
        ...
        if (!brand && !model)
            return { ok: false, error: 'Appareil non reconnu par IMEICheck' };
        return { ok: true, data: { brand: brand || 'Inconnu', model, marketingName } };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        logger_1.logger.warn(`IMEICheck.com TAC error for ${imei}: ${msg}`);
        return { ok: false, error: msg };
    }
}
// ─── Couche 3 fallback — imei.info API (optionnel, clé requise) ───────────────
async function fetchTACFromImeiInfo(imei) {
    const apiKey = process.env.IMEI_INFO_API_KEY;
    if (!apiKey)
        return { ok: false, error: 'IMEI_INFO_API_KEY non configurée' };
    try {
        const res = await axios_1.default.get('https://apiv4.imei.info/', {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger_1.logger.warn(`imei.info API error for ${imei}: ${msg}`);
        return { ok: false, error: msg };
    }
}
...
async function checkBlacklist(imei) {
    ...
    try {
        ...
        const status = res.data?.gsmaBlacklist ?? res.data?.status ?? 'unknown';
        return {
            checked: true,
...
async function checkUniqueness(imei, excludeId) {
    const existing = await prisma_1.prisma.phone.findUnique({ where: { imei } });
    if (!existing)
...
async function validateIMEI(imei, options = {}) {
    const clean = imei.replace(/\s|-/g, '');
    const errors = [];
    const warnings = [];
    // Couche 1 — Format
    ...
    if (format.ok && !luhn.ok)
        errors.push(luhn.error);
...
function quickValidateIMEI(imei) {
    const clean = imei.replace(/\s|-/g, '');
    ...
    if (!luhn.ok)
        return { valid: false, error: luhn.error };
    return { valid: true };
}
